(function ($) {
  $.fn.snsis = function (options) {
		var settings = $.extend({
			...
			This = this,
			controllersElement = $(this).selector;
			methods = {
				sliding: function () {

					$thisDataAction = $(this).data("action");
					$imagesContainerElementNode = $(settings['imagesContainerElement'])[0];
					active = $("img.active",$imagesContainerElementNode).index();
					$("img",$imagesContainerElementNode).eq(active).fadeOut(settings["fadingOutSpeed"], function () {
						$(this).removeClass("active");

						if (active == 0) active = 0;
						else if ($("img",$imagesContainerElementNode).size() - 1 == active) active = -1;

						if ($thisDataAction == "previous") active--;
						else if ($thisDataAction == "next") active++;

						$("img",$imagesContainerElementNode).eq(active).addClass("active").fadeIn(settings["fadingInSpeed"]);
					});
		...
		this.each(function () {
			...
			if (typeof $this.data("action") === "undefined") {
				...
				return false;
			} else if ($.inArray($this.data("action"),["previous", "next"]) == -1) {
		...
		if (settings['enableKeyboardArrows'] === true) {
			...
			$(document).keydown(function (evt) {
				if (evt.keyCode == 37) methods["click"].apply($(controllersElement+"[data-action='previous']"));
				else if (evt.keyCode == 39) methods["click"].apply($(controllersElement+"[data-action='next']"));
			});
		}
